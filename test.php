...
.select-container select {
    ...
    gap: 1em; /* Opcional: espacio entre los elementos */
    height: 60vh;
    height: 100vh;
    overflow: scroll;
}
...
// Consultar categorías, productos y años desde la base de datos
$categorias = $wpdb->get_results("SELECT DISTINCT tag FROM {$wpdb->prefix}news WHERE draft = true");
$productos = $wpdb->get_results("SELECT DISTINCT producto FROM {$wpdb->prefix}news WHERE draft = true");
$anios = $wpdb->get_results("SELECT DISTINCT anio FROM {$wpdb->prefix}news WHERE draft = true");
...
// Unimos todas las condiciones con AND si hay alguna
if (!empty($conditions)) {
...
<div id="mySidenav" class="sidenav">
 <form id="filterForm" method="GET">
    ...
    <div class="sepContainer">
        ...
        <input class="search-news" type="text" id="tituloBusqueda" placeholder="Buscar por título">
    </div>
    ...
    <select id="filtroProducto" class="dropdown">
        <option value="">Categoría</option>
        <?php foreach ($categorias as $categoria): ?>
            <option value="<?php echo $categoria->tag; ?>"><?php echo $categoria->tag; ?></option>
        <?php endforeach; ?>
    </select>
</div>
        <div id="categoriaOpciones"></div> <!-- Aquí se cargarán los checkboxes -->
    </div>
    ...
    <select id="filtroProducto" class="dropdown">
        ...
        <div id="productoOpciones"></div> <!-- Aquí se cargarán los checkboxes -->
    </div>
    <!-- Filtro por Año -->
    <div class="sepContainer">
        <div class="select-container">
    ...
    <select id="filtroProducto" class="dropdown">
        ...
        <div id="anioOpciones"></div> <!-- Aquí se cargarán los checkboxes -->
    </div>
...
// Seleccionar el ícono hamburguesa
...
hamburgerIcon.addEventListener('click', function() {
    ...
    if (newsMenuContainer.style.display === 'block') {
        // Si está visible, lo ocultamos y cambiamos el ícono a hamburguesa
...
document.getElementById('todosButton').addEventListener('click', function() {
    ...
    $('#todosButton').on('click', function() {
        $('#filterForm')[0].reset(); 
        $.ajax({
            url: '<?php echo admin_url("admin-ajax.php"); ?>',
            method: 'GET',
            data: { action: 'quitar_filtros' },
            success: function(response) {
                $('.news-body').html(response); // Mostrar todos los resultados
            }
        });
    });
